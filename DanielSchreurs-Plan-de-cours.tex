...
\begin{figure}[H]
    \begin{center}
        ...
        \caption{Représentation moléculaire des évènements dominants \cite{leclercq2008modele}}
    \end{center}
\end{figure}
\clearpage
\clearpage
...
\begin{table}[H]
    \begin{tabular}{|l|l|}
        ...
        Évaluation des apprentissages & \begin{tabular}[c]{@{}l@{}}Réalisation en classe d'un jeu à 2~dimensions\\ dans un navigateur avec l'API de canvas.\end{tabular}                                                                                                                                                             \\ \hline
    \end{tabular}
\end{table}
\subsection{Justifications}
\begin{itemize}
    \item La compétence que je souhaite entrainer, c'est la réalisation d'interfaces multimédias riches.
    \item Je les y entraine au travers de différentes activités d'apprentissages variées afin de répondre à la différence mathétique des apprenants. Dans tous les cas, toutes ces méthodes visent un même objectif. Construire ensemble les briques nécessaires à la réalisation d'un jeu à 2~dimensions.
    \item Enfin, j'évalue, à la fin, la capacité de l'apprenant à réaliser, en pleine autonomie, un jeu à 2~dimensions dans un navigateur avec l'API de dessin canvas.
\end{itemize}
