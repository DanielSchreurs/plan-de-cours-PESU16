...
\subsection{Justifications}
\begin{itemize}
    \item La compétences que je souhaite entrainer, c'est la réalisation d'interfaces multimédia riches.
    \item J'y entaine mes étudiants au travers de différentes activités d'apprentissages variées afin de répondre à la différence mathétique des étudiants. Dans tous les cas, toutes ces méthodes visent un même objectif. Contruire ensemble les briques nécessaire à la réalisation d'un jeux à 2 dimensions.
    \item Enfin j'évalue à la fin, la capacité de l'étudiant à réaliser en pleine autonomie un jeu à 2 dimension dans navigateur avec l'API de dessin canvas.
\end{itemize}
